	...
	var methods = {
		init:function(options){
			...
			function _create(){
				...
				// TYPE (map/video/..)
				settings.type ? dialogBox.find('.dialog').addClass('dialog-'+settings.type) : false;
				...
				// BUTTONS
				methods.buttons.call(_this, settings.buttons);
		...
		buttons:function(html_buttons){
			if (html_buttons){
				this.data('dialog').dialog.find('.dialog').addClass('dialog-is-buttons');
				this.data('dialog').dialog.find('.dialog-buttons').html(html_buttons);
			}
			return this;
		},
