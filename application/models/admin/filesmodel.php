...
////////////////////////////////////////////////////////////////////// ADD IMAGE
	...
	{	
		...
		if ($image['error'] == 0){
			$file = $image['name'];
			$res = move_uploaded_file($image['tmp_name'], ROOT.$path.'/'.$file);

			if ($res){
				# watermark
				if (isset($_POST['watermark']) && $_POST['watermark']){
					$this->_add_watermark(ROOT.$path.'/'.$file, ROOT.$path.'/'.$file, 'FASHIONDANCE.IN.UA');
...
////////////////////////////////////////////////////////////////////// ADD DIR
	...
	public function addDir()
	{
		...
		# русские символы в windows XP
		//$name = iconv("UTF-8", "WINDOWS-1251", $name);	
		
		if ( ! preg_match('/^root/', $path)){echo'нет root';exit;}
		
		mkdir(ROOT.'/img/'.$path.'/'.$name, 0755);
...
////////////////////////////////////////////////////////////////////// WATERMARK IMAGE
	private function _add_watermark_image($img = '', $out = '', $wm = '')
	{
		...
		if ( ! file_exists($wm)){
			log_message('error', 'Ненайден файл: '.$wm.'. Watermark. filesModel.php');
			return;
		}
...
////////////////////////////////////////////////////////////////////// WATERMARK LETTER
	private function _add_watermark($img = '', $out = '', $text = '', $font = '')
	{
		...
		if ( ! file_exists($font)){
			log_message('error', 'Ненайден файл: '.$font.'. Watermark. filesModel.php');
			return;
		}
		...
		if ( ! $info){
			log_message('error', 'Ошибка getimagesize(): '.$img.'. Watermark. filesModel.php '.__LINE__);
			return;
		}
		...
		if( ! function_exists($fnCreate)){
			log_message('error', 'Ошибка function not exists: '.$fnCreate.'. Watermark. filesModel.php '.__LINE__);
			return;
		}
		
		$img = $fnCreate($img);
		if ( ! $img){
			log_message('error', 'Ошибка '.$fnCreate.'(): '.$img.'. Watermark. filesModel.php '.__LINE__);
			return;
		}
...
////////////////////////////////////////////////////////////////////// WATERMARK ANGLE
	private function _add_watermark_angle($img = '', $out = '', $text = '', $font = '', $r = 128, $g = 128, $b = 128, $alpha = 100)
	{
		$font = $font ? $font : ROOT.'/css/fonts/arial.ttf';
		if ( ! file_exists($font)){
			log_message('error', 'Ненайден файл: '.$font.'. Watermark. filesModel.php '.__LINE__);
			return;
		}
